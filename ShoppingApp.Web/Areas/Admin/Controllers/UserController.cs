    ...
    public class UserController : Controller
    {
        ...
        public async Task<IActionResult> Edit(UserAddDto userUpdateDto)
        {
            if (ModelState.IsValid)
            {
                ...
                var userRoles = await _userManager.GetRolesAsync(user); //user'ın var olan rolleri 
                await _userManager.AddToRolesAsync(
                    user, 
                    userUpdateDto.SelectedRoles.Except(userRoles).ToList<string>());
                //var olan rolleri çıkart kalan rolleri ekle(tekrar var olan rolü ekle yapmamak için)
                await _userManager.RemoveFromRolesAsync(
                    user,
                    userRoles.Except(userUpdateDto.SelectedRoles).ToList<string>()
                    );
                //user rollerin içinden seçili olanları çıkart
                TempData["Message"] = Jobs.CreateMessage("Başarılı", $"{user.UserName} kullanıcısı başarıyla güncellendi.", "success");
                return RedirectToAction("Index", "User");
            }
            //Hatalı olma durumunda ekran tekrar gösterilebilir durumda(bilgilerle)*
            ...
            //Hatalı olma durumunda ekran tekrar gösterilebilir durumda(bilgilerle)*
            userUpdateDto.SelectedRoles = userUpdateDto.SelectedRoles ?? new List<string>(); //rol seçilmezse null dönmesin....çift soru işareti null mı diye bakıyor.... selected roles'ü yarat ama boş yarat.
            return View(userUpdateDto);
        }
